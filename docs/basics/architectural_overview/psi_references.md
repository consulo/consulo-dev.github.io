...
A reference is an instance of a class implementing the [`PsiReference`](https://github.com/consulo/consulo/blob/master/modules/base/language-api/src/main/java/consulo/language/psi/PsiReference.java) interface.
Note that references are distinct from PSI elements.
References created by a PSI element are returned from [`PsiElement.getReferences()`](https://github.com/consulo/consulo/blob/master/modules/base/language-api/src/main/java/consulo/language/psi/PsiElement.java), the underlying PSI element of a reference can be obtained from [`PsiReference.getElement()`](https://github.com/consulo/consulo/blob/master/modules/base/language-api/src/main/java/consulo/language/psi/PsiReference.java).

To *resolve* the reference - to locate the declaration being referenced - call [`PsiReference.resolve()`](https://github.com/consulo/consulo/blob/master/modules/base/language-api/src/main/java/consulo/language/psi/PsiReference.java).
It's very important to understand the difference between [`PsiReference.getElement()`](https://github.com/consulo/consulo/blob/master/modules/base/language-api/src/main/java/consulo/language/psi/PsiReference.java) and [`PsiReference.resolve()`](https://github.com/consulo/consulo/blob/master/modules/base/language-api/src/main/java/consulo/language/psi/PsiReference.java).
...
However, opening this example in Consulo and having a file called "foo.txt" in the same directory, one can <kbd>Ctrl/Cmd</kbd>-Click on "foo.txt" and navigate to the file.
...
Such references return `true` from the [`PsiReference.isSoft()`](https://github.com/consulo/consulo/blob/master/modules/base/language-api/src/main/java/consulo/language/psi/PsiReference.java) method.
...
Such references implement the [`PsiPolyVariantReference`](https://github.com/consulo/consulo/blob/master/modules/base/language-api/src/main/java/consulo/language/psi/PsiPolyVariantReference.java) interface.

For resolving a [`PsiPolyVariantReference`](https://github.com/consulo/consulo/blob/master/modules/base/language-api/src/main/java/consulo/language/psi/PsiPolyVariantReference.java), you call its `multiResolve()` method.
The call returns an array of [`ResolveResult`](https://github.com/consulo/consulo/blob/master/modules/base/language-api/src/main/java/consulo/language/psi/ResolveResult.java) objects.
Each of the objects identifies a PSI element and also specifies whether the result is valid.
For example, suppose you have multiple Java method overloads and a call with arguments not matching any of the overloads. In that case, you will get back [`ResolveResult`](https://github.com/consulo/consulo/blob/master/modules/base/language-api/src/main/java/consulo/language/psi/ResolveResult.java) objects for all of the overloads, and `isValidResult()` returns `false` for all of them.
