...
The Consulo includes a powerful framework for implementing custom language formatters.
...
*  The _formatting model builder_ (
   [`FormattingModelBuilder`](https://github.com/consulo/consulo/blob/master/modules/base/language-code-style-api/src/main/java/consulo/language/codeStyle/FormattingModelBuilder.java) (`consulo.language.codeStyle.FormattingModelBuilder`)
...
*  The formatting model is requested to build the structure of the file as applies to formatting, as a tree of _blocks_ (
   `Block` (`consulo.language.codeStyle.Block`)
...
*  The _spacing_ (`Spacing`) specifies what spaces or line breaks are inserted between the specified children of the block.
...
*  The _wrap_ (`Wrap`) specifies whether the content of the block is wrapped to the next line.
...
*  The _alignment_ (`Alignment`) specifies which blocks should be aligned with each other.
...
Code formatting can be suppressed per region via special comments.
...
Please see `Rearranger` and related for JavaDoc.
