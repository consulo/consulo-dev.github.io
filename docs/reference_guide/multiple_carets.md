...
Support for multiple independent carets has been added to the editor implementation.
...
Core logic related to multi-caret implementation such as accessing currently existing carets, adding and removing carets, is available via [`CaretModel`](https://github.com/consulo/consulo/blob/master/modules/base/code-editor-api/src/main/java/consulo/codeEditor/CaretModel.java) interface, some changes also have been made in [`SelectionModel`](https://github.com/consulo/consulo/blob/master/modules/base/code-editor-api/src/main/java/consulo/codeEditor/SelectionModel.java) interface.
...
* Previously existing methods in `CaretModel` and `SelectionModel` to query and modify caret and selection positions work by default on the primary caret now.
  In the context of [`CaretModel.runForEachCaret`](https://github.com/consulo/consulo/blob/master/modules/base/code-editor-api/src/main/java/consulo/codeEditor/CaretModel.java) method though, they operate on the current caret.
...
When `EditorActionHandler` is invoked, an additional parameter will be passed to it — a caret instance on which it should operate, or `null` if it's invoked without any caret context.
...
`TypedActionHandler` and `TypedHandlerDelegate` implementations are invoked only once for each typed character.
...
Existing actions inheriting from `CodeInsightAction` will work for primary caret only.
To support multiple carets, one should subclass `MultiCaretCodeInsightAction` instead. 
