...
<!-- TODO content: DOM <=> PSI, editor gutter icon->DOM -->
...
```java
interface Root extends consulo.xml.dom.DomElement {
    Foo getFoo();
}

interface Foo extends consulo.xml.dom.DomElement {
    List<Bar> getBars();
}

interface Bar extends consulo.xml.dom.DomElement {
    String getValue();
}
```
...
* Run Consulo with _Plugin DevKit_ enabled in [internal mode](/reference_guide/internal_actions/enabling_internal.md)
...
To suppress spellchecking annotate your DomElement with `@consulo.spellchecker.xml.NoSpellchecking`.
...
But that is rather inconvenient if you want to create the forms in a GUI Designer.
...
Then you bind a GUI Designer form to your subclass and design a beautiful form there.
...
You are welcome to post your questions and comments to our community forums.
