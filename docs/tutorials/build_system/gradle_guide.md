...
Consulo bundles support for Gradle-based development.
...
The Gradle plugin adds Gradle tasks that enable developing Consulo plugins.

When getting started, there are several items to note:
* It is advised to upgrade to the latest available version of the Gradle plugin regularly.
* Also note the minimum required version of Gradle.
* The extended Gradle tasks have descriptions for each task added by the plugin.
  This documentation will focus on the configuration and use four of those tasks:
  * Setup DSL - `intellij { ... }`.
  * Running DSL - `runIde { ... }`
  * Patching DSL - `patchPluginXml { ... }`
  * Publishing DSL - `publishPlugin { ... }`
...
By default, the Gradle plugin will build a plugin project against the Consulo defined by the latest EAP snapshot.
...
Consulo then indexes the build and any associated source code and runtime.

#### Consulo Configuration
Explicitly setting the Setup DSL attributes `intellij.version` and `intellij.type` tells the Gradle plugin to use that configuration of the Consulo to create the plugin project.
...
See the Gradle plugin documentation for information about specifying the plugin and version.
...
Using the corresponding runtime is also the default, so for this use case, no further configuration is required.
...
Setting the Running DSL attribute `runIde.ideDirectory` will define an IDE to be used for the Development Instance.
This attribute is commonly used when running or debugging a plugin in an [alternate Consulo-based IDE](/intro/intellij_platform.md#ides-based-on-the-consulo-platform).

#### Running Against Alternate Versions of the Runtime
Every version of the Consulo has a corresponding version of the [runtime](/basics/ide_development_instance.md#using-a-runtime-for-the-development-instance).
A different version of the runtime can be used by specifying the `runIde.jbrVersion` attribute, describing a version of the runtime that should be used by the IDE Development Instance.
The Gradle plugin will fetch the specified runtime as needed.
...
As mentioned in the section about [configuring the Consulo](#configuring-the-gradle-plugin-for-building-consulo-plugin-projects) used for building plugin projects, the Gradle plugin will fetch the version of the Consulo specified by the default or by the `intellij` attributes.
...
There are controls for managing the `consulo-gradle-plugin` version, and the version of the Gradle itself.
...
A plugin project's `plugin.xml` file has element values that are "patched" at build time from the attributes of the `patchPluginXml` task (Patching DSL).
...
> **TIP** To maintain and generate an up-to-date changelog, consider using a Gradle changelog plugin.

As discussed in [Components of a Wizard-Generated Gradle Consulo Plugin](prerequisites.md#components-of-a-wizard-generated-gradle-consulo-plugin), the Gradle properties `project.version`, `project.group`, and `rootProject.name` are all generated based on the input to the Wizard.
However, the `consulo-gradle-plugin` does not combine and substitute those Gradle properties for the default `<id>` and `<name>` elements in the `plugin.xml` file.
...
- `runPluginVerifier` - runs the Plugin Verifier tool to check the binary compatibility with specified Consulo builds.

Plugin Verifier integration task allows for configuring the exact IDE versions that your plugin will be checked against.
Please check the Plugin Verifier DSL documentation for more details.

### Publishing with the Gradle Plugin
Please review the [Publishing Plugins with Gradle](deployment.md) page before using the Publishing DSL attributes.
...
Consulo plugins have the most straightforward Gradle plugin configuration.
* Determine the version of [Consulo to use for building the plugin project](#configuring-the-gradle-plugin-for-building-consulo-plugin-projects); this is the desired version of the Consulo.
  This can be EAP (default) or determined from the [build number ranges](/basics/getting_started/build_number_ranges.md).
  * If a production version of Consulo is the desired target, set the `intellij` [version attributes](#consulo-platform-configuration) accordingly.
  * Set the necessary [plugin dependencies](#plugin-dependencies), if any.
* If the plugin project should be run or debugged in an IDE Development Instance based on the same Consulo version, no further attributes need to be set for the IDE Development Instance.
  This is the default behavior and is the most common use case.
  * If the plugin project should be run or debugged in an IDE Development Instance based on an alternate version of the Consulo, set the [Running](#running-against-alternate-versions-and-types-of-consulo-based-ides) DSL attribute accordingly.
  * If the plugin project should be run using a runtime other than the default for the IDE Development Instance, specify the [runtime version](#running-against-alternate-versions-of-the-runtime).
