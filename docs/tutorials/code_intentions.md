...
2. Under **IDE Settings**, click **Intentions**. This displays the list of all intention actions currently available in **Consulo**.
...
- How to invoke a quick fix action for a token element under cursor using the `PsiElementBaseIntentionAction` class.
- How to create a JUnit test for this plugin using the `IdeaTestFixtureFactory` class.

## Sample Plugin

The **ConditionalOperatorConverter** sample plugin is available in the `code_samples/conditional_operator_intention` directory.
When launched, this plugin adds the **Convert ternary operator if statement** item to the **Conditional Operator** node in the Consulo Intentions list:
...
1. Start **Consulo** and open the **conditionalOperatorConvertor** plugin project saved into the `code_samples/conditional_operator_intention` directory.
...
If the cursor is positioned on the "?" conditional operator, **Consulo** proposes to replace this conditional (ternary) operator with the "if-then-else" statement:
...
For detailed information about testing and all related procedures, refer to the Testing section of the Consulo documentation.
